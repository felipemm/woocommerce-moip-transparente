			...
			if($resposta->response){
				...
				$payment_form = "
					<script type='text/javascript'>
						...
						function MascaraVencimento(data){
						...
						function changeCreditCardType(tipo){
							...
							switch(tipo.value){
								...
								case 'Mastercard':
									...
									break;
								case 'Hipercard':
									document.getElementById('numeroCartao').maxLength = 18;
									break;
								case 'Visa':
									document.getElementById('numeroCartao').maxLength = 16;
									break;
						...
						function executePayment() {
							...
							btnPagarPedido.style.visibility    = 'none';
							btnCancelarPedido.style.visibility = 'none';
						...
						var funcaoSucesso = function(data){
							...
							if(data.StatusPagamento == 'Sucesso'){
								...
								divResultado.innerHTML = '<img src=\"".esc_url( $woocommerce->plugin_url() )."/assets/images/success.png\" alt=\"Completo!\" style=\"float:left; margin-right: 10px;\"/> Pagamento concluído!';
								divResultado.innerHTML += '<br><br>Sua transação foi processada pelo Moip Pagamentos S/A.';
								divResultado.innerHTML += '<br><br>Caso tenha alguma dúvida referente a transação, entre em contato com o Moip.';
								
								
								//alert(tipoPagamento);
								if(tipoPagamento == 'CartaoCredito'){
									divResultado.innerHTML += 'A sua transação está '+data.Status+' e o código Moip é ' + data.CodigoMoIP;
						...
						var funcaoFalha = function(data) {
							...
							divResultado.innerHTML = '<img src=\"".esc_url( $woocommerce->plugin_url() )."/assets/images/error.gif\" alt=\"Completo!\" style=\"float:left; margin-right: 10px;\"/> Houve um erro ao processar o seu pagamento!';
							divResultado.innerHTML += '<br><br>O MoIP retornou a seguinte mensagem: ".utf8_encode($resposta->error)." ' + JSON.stringify(data);
						};
				...
				$payment_form .= "
						...
						<div id='paymentButtons'>
							<button id='paymentButton' class='button' onclick='executePayment()' style='display: none;'>Pagar com MoIP</button>
							<button id='cancelButton'  class='button' onclick='cancelPayment()'  style='display: none;'>Cancelar Pedido</button>
